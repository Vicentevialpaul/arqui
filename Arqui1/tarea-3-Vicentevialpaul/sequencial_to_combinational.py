...
datos = json.loads(open('circuito_a_traducir.json').read())
#print(datos)
print(datos)
...
def reemplazo_secuencia(lista_secuencia):
    lista_detalle = []
...
nuevo = json.loads(strings, object_hook=lambda dict_obj: [(key, value) for key, value in dict_obj.items()])
print(nuevo)
with open('circuito_equivalente.json', 'w') as file:
